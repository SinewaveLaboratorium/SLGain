...
- **Custom DSP Signal Smoothing**: Implementation of a **First-Order IIR Filter** (`CParamSmooth` class) to handle parameter changes. By calculating coefficients based on the sample rate ($a = e^{-2\pi \cdot F_c}$), the plugin ensures that gain adjustments are click-free and prevent "zipper noise" during real-time processing.
- **Non-Linear Knob Tapering**: A power-curve algorithm (`findShape`) to map linear UI movements to logarithmic audio scales. This ensures the "feel" of the gain knob is consistent and intuitive, regardless of the user-defined dB range.
